...
class JitterTool(object):
  ...
  def cross_correlation_jitter(
    ...
    if verbose == 1:
      ...
      plt.axhline(y=0, lw=0.4, c='darkgrey')
      plt.xlabel('Lag [ms]')
      plt.ylabel('CCG [excess spike pairs]')
      # plt.title('Mean corrected\n'+
  ...
  def allocate_basis_knots(
    ...
    See https://github.com/AlbertYuChen/Divergent_Spiketrain_public/blob/master/
        Code/mylib/makeBasis_StimKernel.m
    """
  ...
  def plot_beta_kernel_samples(
    ...
    plt.xlabel(r'$\hat\beta_h$ [spikes/sec]')
    plt.legend(fontsize=8)

    if output_dir is not None:
      file_path = output_dir + experiment_name + 'hatbeta_h_posterior.pdf'
  ...
  def reconstruct_bspline_basis(
    ...
    axs = None
    if verbose:
      print('filter integral', filter_auc)
      gs_kw = dict(width_ratios=[1], height_ratios=[1])
      fig, axs = plt.subplots(figsize=(5, 3), gridspec_kw=gs_kw,
          nrows=1, ncols=1)
      # plt.subplots_adjust(left=None, right=None, hspace=0, wspace=0)
  ...
  def Sxx_theoretical(
    ...
    if (par['filter_type'] == 'square' and
      ...
      S_hw = mu**2/(2*rho)*cls.erf(sigma_h/np.sqrt(2*sigma_w**2+4*sigma_I**2)) + \
      ...
      S_wlam = mu**2/(rho*np.sqrt(np.pi)*np.sqrt(2*sigma_w**2 + 4*sigma_I**2))
      S_hlam = mu**2/(2*rho)*cls.erf(sigma_h/2/sigma_I)

      par['S_ww_theoretical'] = S_ww * T
      par['S_hh_theoretical'] = S_hh * T
      par['S_hw_theoretical'] = S_hw * T
      par['S_wlam_theoretical'] = S_wlam * T
      par['S_hlam_theoretical'] = S_hlam * T
      par['S_ws_theoretical'] = S_wlam * T + alpha_h * S_hw * T
      par['S_hs_theoretical'] = S_hlam * T + alpha_h * S_hh * T

      par['S_hw_approx'] = T * (S_wlam*sigma_h + 
          bar_lambda_i/np.sqrt(2*np.pi) * (sigma_h/sigma_w))
  ...
  def plot_Sxx_theoretical(
    ...
    #-------------- S_ww --------------
    # The columns are: const, W, h, y.
    S_ww = np.zeros_like(kernel_widths)
    S_hh = np.zeros_like(kernel_widths)
    S_hw = np.zeros_like(kernel_widths)
    S_wlam = np.zeros_like(kernel_widths)
    S_hlam = np.zeros_like(kernel_widths)
    S_ws = np.zeros_like(kernel_widths)
    S_hs = np.zeros_like(kernel_widths)
    S_hw_approx = np.zeros_like(kernel_widths)

    for i, kernel_width in enumerate(kernel_widths):
      par_input = par.copy()
      par_input['kernel_width'] = kernel_width
      par_theoretical = cls.Sxx_theoretical(par_input)
      S_ww[i] = par_theoretical['S_ww_theoretical']
      S_hh[i] = par_theoretical['S_hh_theoretical']
      S_hw[i] = par_theoretical['S_hw_theoretical']
      S_wlam[i] = par_theoretical['S_wlam_theoretical']
      S_hlam[i] = par_theoretical['S_hlam_theoretical']
      S_ws[i] = par_theoretical['S_ws_theoretical']
      S_hs[i] = par_theoretical['S_hs_theoretical']
      # S_hw_approx[i] = par_theoretical['S_hw_approx']
      S_hw_approx[i] = (S_wlam[i]*sigma_h
          + T*bar_lambda_i/np.sqrt(2*np.pi)*sigma_h/kernel_width)

    gs_kw = dict(width_ratios=[1], height_ratios=[1])
    fig, axs = plt.subplots(figsize=(5, 2), gridspec_kw=gs_kw,
        nrows=1, ncols=1)
    ax = fig.add_subplot(axs)
    # plt.plot(np.log10(kernel_widths)+3, S_wlam)
    plt.plot(np.log10(kernel_widths)+3, S_hw)
    plt.plot(np.log10(kernel_widths)+3, S_hw_approx)

    plt.ylim(0, np.max(S_hw) * 1.5)


  @classmethod
  def calculate_Sxx_theoretical_vs_numerical(
      cls,
      spike_times_x, spike_times_y,
      basis_i, basis_j,
      model_par,
    ...
    model_par['background_window'] = 'gaussian'
    model_par['background_window_sigma'] = 0.15
    model_par['window'] = 'gaussian'
    model_par['sigma'] = 0.1
    model_par['baseline'] = 1 # arbitrary.
    model_par['mu'] = bar_lambda_i
    model_par['rho'] = bar_lambda_i
    model_par['alpha'] = [[0,1],[0,0]]  # arbitrary.
    model_par['beta'] = [[0,0.03],[0,0]]  # arbitrary.
    alpha_h = model_par['alpha'][0][1]
    filter_length = model_par['beta'][0][1]
    print(f'num_trials {num_trials}')

    min_kernel_width, max_kernel_width = 0.002, 1
    log_kernel_widths = np.linspace(np.log10(min_kernel_width),
                                    np.log10(max_kernel_width), 200)
    kernel_widths_mod = np.power(10, log_kernel_widths)

    # Theoretical. # The columns are: const, W, h, y.
    S_xx_mod = np.zeros_like(kernel_widths_mod)
    for i, kernel_width in enumerate(kernel_widths_mod):
      # Theoretical
      par_input = model_par.copy()
      par_input['kernel_width'] = kernel_width
      par_theoretical = cls.Sxx_theoretical(par_input)
      if basis_i == 1 and basis_j == 1:
        S_xx_mod[i] = par_theoretical['S_ww_theoretical']
        Sxx_name = 'S_ww'
      elif basis_i == 2 and basis_j == 2:
        S_xx_mod[i] = par_theoretical['S_hh_theoretical']
        Sxx_name = 'S_hh'
      elif basis_i == 1 and basis_j == 2:
        S_xx_mod[i] = par_theoretical['S_hw_theoretical']
        Sxx_name = 'S_hw'
      elif basis_i == 1 and basis_j == 3:
        S_xx_mod[i] = par_theoretical['S_ws_theoretical']
        Sxx_name = 'S_w_lambda + alpha_h x S_hw'
      elif basis_i == 2 and basis_j == 3:
        S_xx_mod[i] = par_theoretical['S_hs_theoretical']
        Sxx_name = 'S_h_lambda + alpha_h x S_hh'
      elif basis_i == 1 and basis_j == 4:  # 4 is virtual, meaning lambda.
        S_xx_mod[i] = par_theoretical['S_wlam_theoretical']
        Sxx_name = 'S_wlam_lambda'
      elif basis_i == 2 and basis_j == 4:  # 4 is virtual, meaning lambda.
        S_xx_mod[i] = par_theoretical['S_hlam_theoretical']
        Sxx_name = 'S_hlam_lambda'

    # Numerical. # The columns are: const, W, h, y.
    kernel_widths_sim = np.array([2, 5, 8, 10, 15, 20, 25, 30, 40, 60, 80, 100, 120,
        140, 160, 200, 250, 300, 500, 800, 1000]) / 1000
    ...
    for k, kernel_width in enumerate(kernel_widths_sim):
      ...
      XY = np.hstack((X, spike_hist))
      S_xx = XY.T @ XY
      for (i,j) in [(1,1),(2,2),(1,2),(1,3),(1,4),(2,4)]:
    ...
    ax = fig.add_subplot(axs)
    # Theoretical.
    plt.plot(np.log10(kernel_widths_mod)+3, S_xx_mod, '-', c='k',
        label='numerical')
    # Numerical.
    plt.plot(np.log10(kernel_widths_sim)+3, Sxx_hat[:,basis_i,basis_j],
        '.-', c='tab:blue', label='numerical')
    plt.title(f'{Sxx_name}')
  ...
  @classmethod
  def plot_Sxx_theoretical_vs_numerical(
      cls,
      basis_i, basis_j,
      par,
      simulation_files,
    ...
    # The columns are: const, W, h, y.
    Sxx_list = np.zeros_like(kernel_widths)
    ...
    for i, kernel_width in enumerate(kernel_widths):
      ...
      par_theoretical = cls.Sxx_theoretical(par_input)
      if basis_i == 1 and basis_j == 1:
        Sxx_list[i] = par_theoretical['S_ww_theoretical']
        Sxx_name = 'S_ww'
      elif basis_i == 2 and basis_j == 2:
        Sxx_list[i] = par_theoretical['S_hh_theoretical']
        Sxx_name = 'S_hh'
      elif basis_i == 1 and basis_j == 2:
        Sxx_list[i] = par_theoretical['S_hw_theoretical']
        Sxx_name = 'S_hw'
      elif basis_i == 1 and basis_j == 3:
        Sxx_list[i] = par_theoretical['S_ws_theoretical']
        Sxx_name = 'S_w_lambda + alpha_h x S_hw'
      elif basis_i == 2 and basis_j == 3:
        Sxx_list[i] = par_theoretical['S_hs_theoretical']
        Sxx_name = 'S_h_lambda + alpha_h x S_hh'
      elif basis_i == 1 and basis_j == 4:  # 4 is virtual, meaning lambda.
        Sxx_list[i] = par_theoretical['S_wlam_theoretical']
        Sxx_name = 'S_wlam_lambda'
      elif basis_i == 2 and basis_j == 4:  # 4 is virtual, meaning lambda.
        Sxx_list[i] = par_theoretical['S_hlam_theoretical']
        Sxx_name = 'S_hlam_lambda'
    ...
    for i, f in enumerate(simulation_files):
      ...
      if i == 0:
        ...
        num_sims = len(model_par_list)
        print(f'num simulation models {num_sims}')
        print('S_xx.shape', S_xx.shape)

      if 'kernel_width' in model_par_list[0]:
        kernel_widths_sim[i] = model_par_list[0]['kernel_width']
        if (basis_i == 1 and basis_j == 3) or (basis_i == 2 and basis_j == 3):
          Sxx_mean_sim[i] = np.mean(S_xx[:,basis_i,3])
          Sxx_std_sim[i] = np.std(S_xx[:,basis_i,3])
        elif (basis_i == 1 and basis_j == 4) or (basis_i == 2 and basis_j == 4):
          S_xx = S_xx[:,basis_i,3] - alpha_h * S_xx[:,basis_i,2] * dt
          Sxx_mean_sim[i] = np.mean(S_xx)
          Sxx_std_sim[i] = np.std(S_xx)
        else:
          Sxx_mean_sim[i] = np.mean(S_xx[:,basis_i,basis_j]) * dt
          Sxx_std_sim[i] = np.std(S_xx[:,basis_i,basis_j]) * dt
      else:
        kernel_widths_sim[i] = np.nan
    ...
    ax = fig.add_subplot(axs)
    # Theoretical.
    plt.plot(np.log10(kernel_widths)+3, Sxx_list, 'k')

    # Numerical
    Sxx_CI = CI_scale * Sxx_std_sim / np.sqrt(num_sims)
    Sxx_CI = np.vstack((Sxx_mean_sim - Sxx_CI, Sxx_mean_sim + Sxx_CI)).T
    print('Sxx mean', np.round(Sxx_mean_sim,-2))
    plt.fill_between(np.log10(kernel_widths_sim)+3, Sxx_CI[:,0], Sxx_CI[:,1],
        facecolor='lightblue', alpha=0.3, label='numerical 95% CI')
    plt.plot(np.log10(kernel_widths_sim)+3, Sxx_mean_sim, '.-', c='tab:blue',
        label='numerical')
    plt.ylim(0, max(Sxx_list)*1.2)
    plt.legend()
    plt.title(Sxx_name)
  ...
  def ks_test(
    ...
    # print('continuous integral ', basis_integral.T)
    # print('X.shape', X.shape)
    # print('num_bins_trial', num_bins_trial)
    # spike_hist_stacked_y, _ = cls.bin_spike_times(spike_times_y, dt,
    #     trial_window=trial_window)
    # spike_hist_y = spike_hist_stacked_y.reshape(-1, 1)
    # basis_integral = basis_integral_discrete
    # beta = np.zeros([num_basis, 1])
  ...
  def dft(
      ...
      xlim=30,
      verbose=True):
    """Perform DFT(FFT) and plot the results."""
    if len(y.shape) == 1: # 1-D array.
      ...
      if verbose:
        print(f'y.shape {y.shape}  spectrum.shape {spectrum.shape}')
        plt.figure(figsize=[20, 2])
        plt.subplot(141)
        plt.plot(freq, spectrum.real)
        plt.plot(freq, spectrum.imag)
        # plt.xlim(-5*xlim, 5*xlim)
        plt.grid()
        plt.subplot(142)
        ...
        plt.grid()
        plt.subplot(144)
        plt.plot(freq, np.abs(spectrum) / N, 'k', lw=1)
        plt.xlim(-xlim, xlim)
        plt.ylim(0)
        plt.grid()
        plt.show()
        signal_energy = y @ y.T
        spectrum_energy = spectrum @ np.conjugate(spectrum) / N
        print('Parseval:', signal_energy, spectrum_energy)
      return spectrum, freq

    if len(y.shape) == 2: # 2-D array. Each row represent a trial.
      N = y.shape[1]
      T = N * dt
      ...
      if verbose:
        ...
        plt.figure(figsize=[20, 2])
        plt.subplot(141)
        plt.plot(freq, np.mean(spectrum, axis=0).real)
        plt.plot(freq, np.mean(spectrum, axis=0).imag)
        # plt.xlim(-5*xlim, 5*xlim)
        plt.grid()
        plt.subplot(142)
        plt.plot(freq, np.mean(spectrum, axis=0).real / N)
        plt.plot(freq, np.mean(spectrum, axis=0).imag / N)
        plt.xlim(-xlim, xlim)
        # plt.ylim(-0.4, 0.4)
        plt.grid()
        plt.subplot(143)
        plt.plot(freq, np.mean(np.abs(spectrum.real), axis=0) / N)
        plt.plot(freq, np.mean(np.abs(spectrum.imag), axis=0) / N)
        plt.xlim(-xlim, xlim)
        plt.ylim(0)
        plt.grid()
        plt.title(f'Median {np.mean(np.abs(spectrum.real)) / N:.5f}  ' +
                  f'{np.mean(np.abs(spectrum.imag)) / N:.5f}')
        plt.subplot(144)
        plt.plot(freq, np.mean(np.abs(spectrum), axis=0) / N, 'k', lw=1)
        # plt.stem(freq, np.mean(np.abs(spectrum), axis=0) / N, 'k')
        plt.xlim(-xlim, xlim)
        plt.ylim(0)
        plt.grid()
        plt.title(f'Median {np.median(np.abs(spectrum))/N:.5f}')
        plt.show()

        signal_energy = np.trace(y @ y.T)
        spectrum_energy = np.trace(spectrum @ spectrum.conj().T) / N
        print('Parseval:', signal_energy, spectrum_energy)
      return spectrum, freq
  ...
  def dft_spike_times(
      ...
      trial_window,
      dt,
      zero_padding_length=0,
      plot_type='average',
      mean_norm=False,
      xlim=25,
      verbose=True):
    """Fourier Transform (discretized) of spike trains.

    Args:
      plot_type: 'average' average across trials.
                 'concatenate' concatenate all trials together.
    """
    trial_length = trial_window[1]
    spike_hist, _ = cls.bin_spike_times(spike_times, dt, trial_length)
    if mean_norm:
      spike_hist = spike_hist - np.mean(spike_hist, axis=1, keepdims=True)
    if len(spike_hist.shape) == 1:
      num_trials = 1
      dimension = '1-1'
    elif len(spike_hist.shape) == 2:
      num_trials = spike_hist.shape[0]
      dimension = '2-2'
    # Zero padding.
    trial_length += zero_padding_length
    num_bins = int(trial_length / dt)
    spike_hist = np.hstack([spike_hist,
        np.zeros([num_trials, int(zero_padding_length/dt)])])

    if plot_type == 'concatenate':
      return cls.dft(spike_hist.reshape(-1), dt, xlim=xlim, verbose=verbose)

    elif plot_type == 'average':
      return cls.dft(spike_hist, dt, xlim=xlim, verbose=verbose)
  ...
  def dft_intensity(
      ...
      xlim=30,
      verbose=True):
    """DFT for the true intensity function."""
    import hierarchical_model_generator
    generator = hierarchical_model_generator.HierarchicalModelGenerator()
    ...
    spectrum_mat = [0] * num_examples
    if num_examples>=4:
      trange = tqdm(range(num_examples), ncols=100, file=sys.stdout)
    else:
      trange = range(num_examples)
    for e in trange:
      peaks = np.random.rand(num_peaks) * trial_length  # Uniform peak positions.
      peaks = np.sort(peaks)
      def intensity_func(t):
        return generator._laplacian_intensity_func(t, peaks, sigma, baseline)

      # Concatenate all trials into one spike train.
      T = trial_length
      t_sample = np.arange(0, T, dt)
      y = intensity_func(t_sample) * dt
      if mean_norm:
        y = y - np.mean(y)
      t = np.arange(0, T+zero_padding_length, dt)
      y = np.concatenate([y, np.zeros(int(zero_padding_length/dt))])
      spectrum = np.fft.fft(y.reshape(-1))
      N = int((T+zero_padding_length)/dt)
      spectrum_mat[e], freq = cls.dft(y, dt, xlim=xlim, verbose=num_examples<4)
      # cls.dft(y, dt, N//2, xlim=xlim)

    spectrum_mat = np.stack(spectrum_mat, axis=0)
    if num_examples >= 4:
      # CI_scale = scipy.stats.norm.ppf(0.975)  # 95% CI.
      CI_scale = 1
      spectrum_mean = np.mean(np.abs(spectrum_mat), axis=0)
      spectrum_std = np.std(np.abs(spectrum_mat), axis=0)

      plt.figure(figsize=[5, 3])
      # plt.plot(freq, np.abs(spectrum_mat).T, 'k', lw=1)
      plt.plot(freq, spectrum_mean, 'k', lw=2)
      plt.fill_between(freq, spectrum_mean+spectrum_std*CI_scale,
                       spectrum_mean-spectrum_std*CI_scale,
  ...
  def dft_window(
    ...
    sigma = model_par['sigma']
    def intensity_func(t):
      return generator._laplacian_intensity_func(t, [0], sigma, baseline)

    T = trial_length
    t = np.arange(-T/2, T/2, dt)
    N = int(T/dt)

    y = intensity_func(t) * dt
    cls.dft(y, dt, N//2, xlim=xlim)

    y = np.exp(-np.abs(20*t))
    cls.dft(y, dt, N//2, xlim=xlim)
    ...
    y = np.zeros_like(t)
    y[(t > -0.1) & (t < 0.1)] = 1
    y = np.convolve(y, y, 'same')
    cls.dft(y, dt, N//2-2, xlim=xlim)
  ...
  def dft_lambda(
    ...
    print('X.shape', X.shape)
    beta = model_par['beta']
    lambda_hat = (X @ beta).reshape(-1) * dt

    if stack_type == 'concatenate':
      if mean_norm:
        lambda_hat -= np.mean(lambda_hat)
      return cls.dft(lambda_hat, dt, xlim=xlim, verbose=verbose)

    elif stack_type == 'average':
      lambda_hat = lambda_hat.reshape(num_trials, -1)
      if mean_norm:
        lambda_hat = lambda_hat - np.mean(lambda_hat, axis=1, keepdims=True)
      return cls.dft(lambda_hat, dt, xlim=xlim, verbose=verbose)


  @classmethod
  def dft_phi_div_lambda(
      cls,
      spike_times_x,
      spike_times_y,
      beta_id,
    ...
    beta = model_par['beta']
    lambda_hat = (X @ beta).reshape(-1)
    y = X[:,beta_id] / lambda_hat

    if stack_type == 'concatenate':
      if mean_norm:
        y -= np.mean(y)
      return y, cls.dft(y, dt, xlim=xlim, verbose=verbose)
  ...
  def dft_phi(
    ...
    if stack_type == 'concatenate':
      if mean_norm:
        y -= np.mean(y)
      return cls.dft(y.reshape(-1), dt, xlim=xlim, verbose=verbose)
  ...
  def dft_residual(
    ...
    lambda_hat = (X @ beta).reshape(-1)
    spike_hist_stacked, _ = cls.bin_spike_times(spike_times_y, dt, trial_length)
    spike_hist = spike_hist_stacked.reshape(-1)
    y = spike_hist - lambda_hat * dt
    print('X.shape', X.shape)
    print('sum spikes, lambda ', np.sum(spike_hist), np.sum(lambda_hat)*dt)

    # Equivalent to row wise dot product then take the sum.
    nll = - np.sum(spike_hist * np.log(lambda_hat))
    nll += lambda_hat.sum() * dt
    print('nll:', nll)
    ...
    elif stack_type == 'average_spectrum_diff':
      spectrum1, freq = cls.dft_spike_times(spike_times_y,
          trial_window, dt, zero_padding_length=0, mean_norm=True,
          xlim=xlim, plot_type='average', verbose=verbose)
      spectrum2, _    = cls.dft_lambda(spike_times_x, spike_times_y,
          trial_window, dt, model_par, zero_padding_length=0, mean_norm=True,
          xlim=xlim, stack_type='average', verbose=verbose)
      spectrum_res = spectrum1 - spectrum2

      plt.figure(figsize=[15, 2])
      plt.subplot(131)
      plt.plot(freq, np.mean(spectrum_res.real, axis=0)/spectrum_res.shape[1])
      plt.plot(freq, np.mean(spectrum_res.imag, axis=0)/spectrum_res.shape[1])
      plt.xlim(-xlim, xlim)
      plt.subplot(132)
      plt.plot(freq, np.mean(np.abs(spectrum1), axis=0)/spectrum1.shape[1])
      plt.plot(freq, np.mean(np.abs(spectrum2), axis=0)/spectrum2.shape[1])
      plt.xlim(-xlim, xlim)
      plt.ylim(0, 0.01)
      plt.subplot(133)
      plt.plot(freq, np.mean(np.abs(spectrum_res), axis=0)/spectrum_res.shape[1])
      plt.xlim(-xlim, xlim)
      plt.ylim(0)
      plt.show()

      return spectrum_res, freq


  @classmethod
  def dft_dirivative_log_likelihood(
      cls,
      spike_times_x,
      spike_times_y,
      beta_id,
      ...
      stack_type='average',
      verbose=False):
    """DFT of Phi / lambda_hat * (spikes - lambda_hat)."""
    num_trials = len(spike_times_y)
    # series1, (spectrum1,_) = cls.dft_phi(spike_times_x, spike_times_y,
    #     beta_id, trial_window, dt, model_par, zero_padding_length,
    #     mean_norm=False, stack_type=stack_type, xlim=xlim, verbose=verbose)

    series1, (spectrum1,_) = cls.dft_phi_div_lambda(spike_times_x, spike_times_y,
        beta_id, trial_window, dt, model_par, zero_padding_length,
        mean_norm=False, stack_type=stack_type, xlim=xlim, verbose=verbose)
    
    series2, (spectrum2,_) = cls.dft_residual(spike_times_x, spike_times_y,
        trial_window, dt, model_par, zero_padding_length,
        mean_norm=False, stack_type=stack_type, xlim=xlim, verbose=verbose)
    ...
    if stack_type == 'concatenate':
      if spectrum_range is not None:
        spectrum1 = np.hstack((spectrum1[:spectrum_range], spectrum1[-spectrum_range:]))
        spectrum2 = np.hstack((spectrum2[:spectrum_range], spectrum2[-spectrum_range:]))

      print('spectrum.shape', spectrum1.shape)
      T = trial_window[1]
      N = int((T+zero_padding_length)/dt) * num_trials
      derivative_ll_spectrum = spectrum1 @ spectrum2.conj() / N
      derivative_ll = series1 @ series2
      print('d LL:', derivative_ll, derivative_ll_spectrum)
      energy1 = spectrum1 @ spectrum1.conj() / N
      energy2 = spectrum2 @ spectrum2.conj() / N
      print('spectrum energy:', energy1, energy2)
      ...
      T = trial_window[1]
      N = int((T+zero_padding_length)/dt)
      derivative_ll_spectrum = np.trace(spectrum1 @ spectrum2.conj().T) / N
      derivative_ll = np.trace(series1 @ series2.T)
      energy1 = np.trace(spectrum1 @ spectrum1.conj().T) / N
      energy2 = np.trace(spectrum2 @ spectrum2.conj().T) / N

      print('\nD LL:', derivative_ll, '  F LL:', derivative_ll_spectrum)
      print('spectrum energy:', energy1, energy2)
  ...
  @classmethod
  def plot_continuous_time_multivariate_regression_model_par(
      cls,
      model_par_list,
      filter_par=None,
      ylim=None,
    ...
    for n in range(num_x):
      ...
      if filter_par is not None and filter_par['type'] == 'triangle':
        ...
        plt.ylim(ylim)
      if filter_par is not None and filter_par['type'] == 'square':
        filter_alpha = filter_par['alpha'][0][num_x-n]
        filter_beta = filter_par['beta'][0][num_x-n]
        plt.plot([0, filter_beta], [filter_alpha, filter_alpha], 'g',
            label='True filter')
        plt.ylim(ylim)
        plt.xlim(0)
      ...
      # plt.xlabel('Time [ms]')
      # plt.ylabel('Firing rate [spk/sec]')
      plt.text(0.8,0.85, f'{n}', transform=ax.transAxes)
      plt.ylim(ylim)
      plt.xlim(0)
